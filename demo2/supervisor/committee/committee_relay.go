...
func (rthm *RelayCommitteeModule) MsgSendingControl() {
	log.Println("[Relay] MsgSendingControl: start reading and injecting transactions...")
	...
	for {
		...
		if len(txlist) == int(rthm.batchDataNum) || rthm.nowDataNum == rthm.dataTotalNum {
			batchCount++
			log.Printf("[Relay] Sending batch %d, txs in batch: %d, total sent: %d / %d\n", batchCount, len(txlist), rthm.nowDataNum, rthm.dataTotalNum)
	...
	log.Printf("[Relay] MsgSendingControl: all %d transactions sent. Now waiting for nodes to finish processing...\n", rthm.nowDataNum)
}
