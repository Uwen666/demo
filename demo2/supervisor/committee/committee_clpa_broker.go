...
func (ccm *CLPACommitteeMod_Broker) MsgSendingControl() {
	...
	for {
		...
		if params.ShardNum > 1 && !ccm.clpaLastRunningTime.IsZero() && time.Since(ccm.clpaLastRunningTime) >= time.Duration(ccm.clpaFreq)*time.Second {
			ccm.sl.Slog.Printf("[DEBUG] CLPA timer fired! elapsed=%v, clpaFreq=%d s, triggering partition clpaCnt=%d\n", time.Since(ccm.clpaLastRunningTime), ccm.clpaFreq, clpaCnt+1)
			ccm.clpaLock.Lock()
			clpaCnt++
			...
			ccm.sl.Slog.Printf("[DEBUG] Waiting for curEpoch to reach %d (current=%d)\n", clpaCnt, atomic.LoadInt32(&ccm.curEpoch))
			for atomic.LoadInt32(&ccm.curEpoch) != int32(clpaCnt) {
				time.Sleep(time.Second)
				ccm.sl.Slog.Printf("[DEBUG] Still waiting... curEpoch=%d, target=%d\n", atomic.LoadInt32(&ccm.curEpoch), clpaCnt)
			}
			ccm.clpaLastRunningTime = time.Now()
...
func (ccm *CLPACommitteeMod_Broker) clpaMapSend(m map[string]uint64) {
	...
	// send to all nodes in each shard (not just NodeID=0) to handle view change
	for i := uint64(0); i < uint64(params.ShardNum); i++ {
		for j := uint64(0); j < uint64(params.NodesInShard); j++ {
			go networks.TcpDial(send_msg, ccm.IpNodeTable[i][j])
		}
	}
...
func (ccm *CLPACommitteeMod_Broker) HandleBlockInfo(b *message.BlockInfoMsg) {
	ccm.sl.Slog.Printf("received from shard %d in epoch %d. (curEpoch=%d)\n", b.SenderShardID, b.Epoch, atomic.LoadInt32(&ccm.curEpoch))
	if atomic.CompareAndSwapInt32(&ccm.curEpoch, int32(b.Epoch-1), int32(b.Epoch)) {
		ccm.sl.Slog.Printf("[DEBUG] curEpoch updated to %d by shard %d\n", b.Epoch, b.SenderShardID)
	}
