...
func (rbhm *RawBrokerPbftExtraHandleMod) HandleinCommit(cmsg *message.Commit) bool {
	...
	leaderID := uint64(rbhm.pbftNode.view.Load()) % rbhm.pbftNode.node_nums
	if rbhm.pbftNode.NodeID == leaderID {
		...
		for sid := uint64(0); sid < rbhm.pbftNode.pbftChainConfig.ShardNums; sid++ {
			...
			for nid := uint64(0); nid < rbhm.pbftNode.pbftChainConfig.Nodes_perShard; nid++ {
				go networks.TcpDial(msg_send, rbhm.pbftNode.ip_nodeTable[sid][nid])
			}
			rbhm.pbftNode.pl.Plog.Printf("S%dN%d : sended sequence ids to %d\n", rbhm.pbftNode.ShardID, rbhm.pbftNode.NodeID, sid)
		}
		txPoolSize := rbhm.pbftNode.CurChain.Txpool.GetTxQueueLen()
