...
// ViewChangeData is the key used to bucket view-change votes.
// SeqID is intentionally excluded: after an epoch reconfig different nodes
// may have ended up with different sequence IDs (some received CReconfig,
// some did not).  Including SeqID would split votes across buckets so the
// quorum of 2f+1 is never reached.  NextView alone uniquely identifies the
// target view transition within one epoch.
