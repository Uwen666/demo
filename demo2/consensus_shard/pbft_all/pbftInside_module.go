...
func (rphm *RawRelayPbftExtraHandleMod) HandleinCommit(cmsg *message.Commit) bool {
	...
	leaderID := uint64(rphm.pbftNode.view.Load()) % rphm.pbftNode.node_nums
	if rphm.pbftNode.NodeID == leaderID {
		...
		txPoolSize := rphm.pbftNode.CurChain.Txpool.GetTxQueueLen()
		...
		bim := message.BlockInfoMsg{
			...
			Epoch:           int(rphm.pbftNode.currentEpoch.Load()),
