...
// ResetToGenesis resets the blockchain to a fresh genesis block.
// Used after cross-shard reconfiguration so all nodes in the new shard
// start from the same common state regardless of their previous shard history.
func (bc *BlockChain) ResetToGenesis() {
	genisisBlock := bc.NewGenisisBlock()
	bc.CurrentBlock = genisisBlock
	bc.Storage.AddBlock(genisisBlock)
	bc.Txpool = core.NewTxPool()
	bc.PartitionMap = make(map[string]uint64)
}

// ResetToGenesisPreserveTxPool resets the blockchain state but keeps the
// existing transaction pool intact. Used during epoch reconfiguration so that
// pending transactions accumulated before the epoch boundary are not lost.
func (bc *BlockChain) ResetToGenesisPreserveTxPool() {
	genisisBlock := bc.NewGenisisBlock()
	bc.CurrentBlock = genisisBlock
	bc.Storage.AddBlock(genisisBlock)
	// Txpool is intentionally NOT reset here — preserve unprocessed transactions
	bc.PartitionMap = make(map[string]uint64)
}
